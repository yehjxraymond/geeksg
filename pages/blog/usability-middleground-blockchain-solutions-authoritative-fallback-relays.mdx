...
Interoperability is a problem which keeps presenting itself when designing a blockchain solution. To solve the problem of an interoperable blockchain, often more blockchains are created, leading to standards and products proliferation. Of which, hardly any are usable for the regular user on the street. 
...
Mimicking existing interface, data schema or artefacts is one way we can introduce familiarity to our user. When designing a solution, we need to take an approach where users using your product will be worse off, not even in scenarios where they have to interact with users of other products. In this case, it will be in your interest, as a product owner, to have a fallback mechanism to more familiar processes or artefacts. 

Falling back to an existing non-blockchain solution does not mean creating another suite of 'general relay blockchain to solve all integration problem'. Instead, these relays need to be hyperspecific and tightly coupled with the use case and must ingest or output real on-the-ground artefacts. In addition, the entity running the relay must be identifiable and be held legally accountable for their duties.
...
If the exchange does not have a BTC/SGD pair, it will not work. If the exchange does not support sending 100k to a specific bank in Singapore, it will not work. If the exchange is not connected to the BTC network or the bank I'm a customer of (directly or indirectly), it will not work. 
...
Let's see how we can use Authoritative Fallback Relay (AFR) in a use case close to my work at OpenCerts, verifiable claims.
...
For this implementation, we can use [Adobe's automatic PDF digital signature validation](https://helpx.adobe.com/acrobat/using/validating-digital-signatures.html). We can create a simple serverless function that:
...
- applying for jobs in companies that are not familiar with more secured credentials format
...
One example is for several institutes to share a printing service which one can receive his printed certificate with his certificate file. This printing service could even be an online service which you can upload the document, make payment for the printing and the certificate will be sent to your address! 
...
Notice how ridiculous validation of physical certificates sounds? Let's move on.
