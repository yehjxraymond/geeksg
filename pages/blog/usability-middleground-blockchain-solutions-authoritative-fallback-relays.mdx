...
export const meta = {
  ...
  summary: "A product's usability cannot be measured without the context of the user. In the case of blockchain solutions, the consumer could be other machines or programs as well! As a result, a blockchain product will have a different degree of usability to a human user, another program using the same standards and interface, and another program not using the same standards and interface"
};
...
When I look at existing products, many are hardly designed for human users or other machines that do not use the same standards & interface. There is often an expectation that the product will naturally work when everyone is using it and everyone should adopt the new methodology or process.
...
Let's see how we can use Authoritative Fallback Relay (AFR) in a use case close to my work at OpenCerts, on verifiable claims.
